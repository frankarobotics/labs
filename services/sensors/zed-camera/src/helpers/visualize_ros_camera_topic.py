...
class CameraViewer(Node):  # type: ignore
    ...
    def __init__(self, topic: str, msg_type: str = "Image") -> None:
        ...
        self.frame: np.ndarray[Any, np.dtype[Any]] | None = None
        self.msg_type = msg_type
        if msg_type == "CompressedImage":
            self.subscription = self.create_subscription(CompressedImage, topic, self.compressed_callback, 10)
        else:
            self.subscription = self.create_subscription(Image, topic, self.raw_callback, 10)
