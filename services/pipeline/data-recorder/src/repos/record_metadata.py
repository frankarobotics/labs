...
class RecordMetadataRepo:
    """Thread-safe repository for CRUD operations on record metadata JSON files."""
    ...
    def _get_file_path(self, episode_id: UUID) -> Path:
        ...
        return Path(
            f"{self.base_path}/{dt.strftime('%Y')}/{dt.strftime('%m')}/{dt.strftime('%d')}/{episode_id}/record_metadata.json"
        )

    def create(self, metadata: RecordMetadata) -> bool:
    ...
    def read(self, episode_id: UUID) -> RecordMetadata | None:
    ...
    def update(self, metadata: RecordMetadata) -> bool:
