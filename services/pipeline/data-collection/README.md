...
The service exposes a FastAPI control plane that keeps human-operated teleop sessions, robot actuators, and observer sensors in sync. It owns the canonical episode metadata in Postgres, loads task definitions from YAML configuration, proxies start/stop commands to the Data Recorder service, and keeps device health up to date for UI consumption.
...
2. **Episode management**: `EpisodeService` talks to Postgres, the metadata repo, and the Data Recorder HTTP API to create, stop, save, discard, and label episodes.
3. **Device health tracking**: `DeviceMonitor` mirrors the station configuration into the DB and continuously inspects ROS topics to mark devices ONLINE/OFFLINE/UNKNOWN.
...
- **FastAPI application** (`src/main.py`): Creates the app with a lifespan hook that blocks on database readiness, initializes rclpy, launches the `DeviceMonitor`, wires CORS, and mounts routers from `handlers/`.
...
- **Repositories** (`src/repos/`): Typed adapters for Postgres entities plus HTTP clients such as `DataRecorderRepo`.
