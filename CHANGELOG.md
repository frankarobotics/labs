...
- Introduced an `AUTORECOVERY` state in the controller coordinator and the workflow state machine. When a controller
  dies during `READY`/`SYNCING`/`FOLLOWING`, the coordinator attempts to re-activate the hardware interface and the
  ready controller and returns to `READY`, falling back to `IDLE` on failure. The workflow mirrors this across all
  coordinators. A toast notification is shown in the UI while recovery is in progress.
